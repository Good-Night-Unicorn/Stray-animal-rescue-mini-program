        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '求助状态' : ''">
                        <el-select v-model="searchForm.qiuzhuzhuangtaiTypes" placeholder="请选择求助状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in qiuzhuzhuangtaiTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('dongwu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/liulangdonwujiuzhu/upload/dongwuMuBan.xls"
                        >批量导入动物求助数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('dongwu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="dongwuUploadSuccess"
                                :on-error="dongwuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('dongwu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入动物求助数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('dongwu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "dongwu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="求助热度">
                        <template slot-scope="scope">
                            {{scope.row.dongwuClicknum}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="qiuzhuzhuangtaiTypes"
                                      header-align="center"
                                      label="求助状态">
                        <template slot-scope="scope">
                            {{scope.row.qiuzhuzhuangtaiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('dongwu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('dongwu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('dongwu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'dongwu',//当前表表名,
                        sumColum : 'dongwu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'dongwu_name',//分组字符串字段
                        // types : 'dongwu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['dongwuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            dongwuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "dongwu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
