	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chongwuLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chongwuLiuyan', this.chongwuLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.chongwuLiuyanPhoto ? this.detail.chongwuLiuyanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('chongwuLiuyanId', _this.chongwuLiuyanId);
                                // _this.$utils.jump('/pages/chongwuLiuyanYuyue/add-or-update');

								await _this.$api.add('chongwuLiuyanYuyue', {
									yonghuId: _this.user.id,
									chongwuLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/chongwuLiuyanYuyue/list');
							}
						}
					});
				},
