	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.dongwuYuyueText" type="text" placeholder="领养缘由"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dongwuYuyue','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('dongwuYuyue','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.dongwuYuyueName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('dongwuYuyue','删除') && item.yonghuId==userId" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.dongwuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">求助名称</text>
                    <text style="width: 70%">{{item.dongwuName}}</text>
                </view>
                <view v-show="item.dongwuTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">求助类型</text>
                    <text style="width: 70%">{{item.dongwuValue}}</text>
                </view>
                <view v-show="item.yonghuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">用户名称</text>
                    <text style="width: 70%">{{item.yonghuName}}</text>
                </view>
                <!-- 当前表 -->
                <view v-show="item.dongwuYuyueText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">领养缘由</text>
                    <view style="width: 70%" v-html="item.dongwuYuyueText"></view>
                </view>
                 <view v-show="item.dongwuYuyueYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">审核状态</text>
                     <text style="width: 70%">{{item.dongwuYuyueYesnoValue}}</text>
                 </view>
                <view v-show="item.dongwuYuyueYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核回复</text>
                    <view style="width: 70%" v-html="item.dongwuYuyueYesnoText"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('dongwuYuyue','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <uni-icons v-show="isAuth('dongwuYuyue','回复')" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
                <view v-if="isAuth('dongwuYuyue','审核') && item.dongwuYuyueYesnoTypes==1 && item.dongwuYonghuId==userId" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`dongwuYuyue`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dongwuYuyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.dongwuYuyueShenheTime=shijian;
                            await _this.$api.update(`dongwuYuyue`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dongwuYuyueText) {
                    searchForm['dongwuYuyueText'] = this.searchForm.dongwuYuyueText
                }

                let res = await this.$api.page(`dongwuYuyue`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
