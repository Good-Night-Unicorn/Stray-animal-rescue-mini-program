	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dongwuYuyue', this.dongwuYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dongwuYuyuePhoto ? this.detail.dongwuYuyuePhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('dongwuYuyueId', _this.dongwuYuyueId);
                                // _this.$utils.jump('/pages/dongwuYuyueYuyue/add-or-update');

								await _this.$api.add('dongwuYuyueYuyue', {
									yonghuId: _this.user.id,
									dongwuYuyueId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/dongwuYuyueYuyue/list');
							}
						}
					});
				},
