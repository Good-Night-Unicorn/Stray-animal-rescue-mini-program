        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx","height":"200rpx"}'
                          :disabled="ro.dongwuYuyueText" v-model="ruleForm.dongwuYuyueText" placeholder="领养缘由"/>
            </view>
    ...
    export default {
        data() {
            ...
            ruleForm: {
					...
					yonghuId: '',
					dongwuYuyueText: '',
					dongwuYuyueYesnoTypes: '',//数字
					dongwuYuyueYesnoValue: '',//数字对应的值
		...
		/*下拉框*/
			let dongwuYuyueYesnoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'dongwu_yuyue_yesno_types',
            }
			let dongwuYuyueYesnoTypes = await this.$api.page(`dictionary`, dongwuYuyueYesnoTypesParams);
			this.dongwuYuyueYesnoTypesOptions = dongwuYuyueYesnoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`dongwuYuyue`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.dongwuYuyueId){
                this.ruleForm.dongwuYuyueId = options.dongwuYuyueId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            // 下拉变化
            dongwuYuyueYesnoTypesChange(e) {
                this.dongwuYuyueYesnoTypesIndex = e.target.value
                this.ruleForm.dongwuYuyueYesnoValue = this.dongwuYuyueYesnoTypesOptions[this.dongwuYuyueYesnoTypesIndex].indexName
                this.ruleForm.dongwuYuyueYesnoTypes = this.dongwuYuyueYesnoTypesOptions[this.dongwuYuyueYesnoTypesIndex].codeIndex
            },
            // styleChange() {
            // 	this.$nextTick(() => {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.dongwuYuyueText)) {
                    this.$utils.msg(`领养缘由不能为空`);
                    return
                }
                if(uni.getStorageSync('dongwuId')){
                    this.ruleForm.dongwuId = uni.getStorageSync('dongwuId')
                }

                if (this.ruleForm.id) {
                    await this.$api.update(`dongwuYuyue`, this.ruleForm);
                } else {
                    await this.$api.save(`dongwuYuyue`, this.ruleForm);
                }
