	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dongwuCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dongwuCollection', this.dongwuCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dongwuCollectionPhoto ? this.detail.dongwuCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('dongwuCollectionId', _this.dongwuCollectionId);
                                // _this.$utils.jump('/pages/dongwuCollectionYuyue/add-or-update');

								await _this.$api.add('dongwuCollectionYuyue', {
									yonghuId: _this.user.id,
									dongwuCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/dongwuCollectionYuyue/list');
							}
						}
					});
				},
