<template>
			<view style="margin: 0 30rpx;box-shadow:0 0px 0px 1rpx #a6a6a6">
				...
				<view style="font-weight: bold;font-size: 20upx;margin:0 30rpx 5rpx;"  :style="(detail.newsValue == null || detail.insertTime == null)?'text-align: center;':''">
					<text v-if="detail.newsValue">
						{{detail.newsValue}}
					</text>
					<text :style="detail.newsValue?'float: right':''">
						{{detail.insertTime}}<!-- .split(' ')[0] -->
					</text>
				</view>
				<view style="box-shadow: 0 0  0 1rpx #a6a6a6;margin: 0 30rpx"></view>
                <viwe>
					<image :src="baseUrl+detail.newsPhoto"></image>
                </viwe>
				<view class="content">
					<rich-text :nodes="detail.newsContent"></rich-text>
				</view>
			</view>
	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('news', this.newsId);
                    this.detail = res.data
					this.detail.newsContent = this.detail.newsContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('newsId', _this.newsId);
                                // _this.$utils.jump('/pages/newsYuyue/add-or-update');

								await _this.$api.add('newsYuyue', {
									yonghuId: _this.user.id,
									newsId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/newsYuyue/list');
							}
						}
					});
				},
