			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.dongwuName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.dongwuContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<view v-if="detail.qiuzhuzhuangtaiTypes == 1" @tap="baoming" style="display: flex; width: 80%;color: #FFFFFF;">
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dongwuId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getdongwuCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        dongwuId: this.dongwuId,
                        yonghuId: this.user.yonghuId,
						dongwuLiuyanPingfenNumber: this.scoreData,
                        dongwuLiuyanText: this.commentData,
                    }
                    await this.$api.save(`dongwuLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dongwu', this.dongwuId);
                    this.detail = res.data
					this.detail.dongwuContent = this.detail.dongwuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.dongwuPhoto ? this.detail.dongwuPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getdongwuCollection() {
					let params = {
						page: 1,
						limit: 1,
						dongwuId: this.dongwuId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`dongwuCollection`, params);
					if (res.data.list.length == 1) {
						let dongwuCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('dongwuCollection', JSON.stringify([dongwuCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getdongwuCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dongwuCollection', {
									yonghuId: _this.user.id,
									dongwuId: _this.detail.id,
									dongwuCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('dongwuLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						dongwuId: this.dongwuId
					});
