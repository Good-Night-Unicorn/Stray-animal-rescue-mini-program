	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.dongwuName" type="text" placeholder="求助名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dongwu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('dongwu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='dongwuTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in dongwuTypesList" :key="index" :class="dongwuTypes===item.codeIndex?'tabActive':''"
                                  @tap="dongwuTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list-box5"
      :style='{"padding":"24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'
      style="box-sizing: border-box;">
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
              :style='{"padding":"0 20rpx 0 0","boxShadow":" 0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"14rpx 0","borderColor":"red","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"100rpx 40rpx 40rpx 100rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'
              style="display: flex;">
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"100rpx","borderWidth":"10rpx","width":"260rpx","borderStyle":"solid","height":"200rpx"}'
                    mode="aspectFill" :src="product.dongwuPhoto?baseUrl+product.dongwuPhoto.split(',')[0]:''"></image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        :style='{"padding":"12rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                        class="title hide1">{{product.dongwuName}}
                </view>
                <view v-if="product.dongwuNewMoney"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"32rpx","borderStyle":"solid"}'
                      class="title">￥{{product.dongwuNewMoney}}
                </view>
                <view v-else-if="product.dongwuValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title">{{product.dongwuValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx;">
                    <text v-if="isAuth('dongwu','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('dongwu','删除')" class="cuIcon-delete"
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==1" class="list-item"
              :style='{"padding":"0 0 0 20rpx","boxShadow":"0px 6rpx 12rpx rgba(0, 0, 0, 0.16)","margin":"14rpx 0","borderColor":"red","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"40rpx 100rpx 100rpx 40rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'
              style="display: flex;">
            <view class="list-item-body" style="flex: 1;">
                <view
                        :style='{"padding":"12rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 求助类型 字段 字典表查询方法
            async dongwuTypesSelect(){
                let dongwuTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'dongwu_types',
                });
                dongwuTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.dongwuTypesList = dongwuTypes.data.list
            },
            //当前表的 求助类型字段类别搜索
			dongwuTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.dongwuTypes != 0) {
                    params.dongwuTypes = this.dongwuTypes
                }
                let _this = this
                let res = await this.$api.page(`dongwu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dongwu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.dongwuShenheTime=shijian;
                            await _this.$api.update(`dongwu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dongwuName) {
                    searchForm['dongwuName'] = this.searchForm.dongwuName
                }

                let res = await this.$api.page(`dongwu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.dongwuContent!=null){
                        item.dongwuContent = item.dongwuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
