			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.chongwuName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.chongwuContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chongwuId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getchongwuCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        chongwuId: this.chongwuId,
                        yonghuId: this.user.yonghuId,
						chongwuLiuyanPingfenNumber: this.scoreData,
                        chongwuLiuyanText: this.commentData,
                    }
                    await this.$api.save(`chongwuLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chongwu', this.chongwuId);
                    this.detail = res.data
					this.detail.chongwuContent = this.detail.chongwuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.chongwuPhoto ? this.detail.chongwuPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getchongwuCollection() {
					let params = {
						page: 1,
						limit: 1,
						chongwuId: this.chongwuId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`chongwuCollection`, params);
					if (res.data.list.length == 1) {
						let chongwuCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('chongwuCollection', JSON.stringify([chongwuCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getchongwuCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('chongwuCollection', {
									yonghuId: _this.user.id,
									chongwuId: _this.detail.id,
									chongwuCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('chongwuLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						chongwuId: this.chongwuId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('chongwu', _this.detail);
									_this.$utils.msg('取消成功');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('chongwuId', _this.chongwuId);
                                // _this.$utils.jump('/pages/chongwuYuyue/add-or-update');

								await _this.$api.add('chongwuYuyue', {
									yonghuId: _this.user.id,
									chongwuId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/chongwuYuyue/list');
							}
						}
					});
				},
