	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.chongwuName" type="text" placeholder="寻宠标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('chongwu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('chongwu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='chongwuTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in chongwuTypesList" :key="index" :class="chongwuTypes===item.codeIndex?'tabActive':''"
                                  @tap="chongwuTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.chongwuPhoto?baseUrl+product.chongwuPhoto.split(',')[0]:''"></image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.chongwuName}}
                </view>
                <view v-if="product.chongwuNewMoney"
                      ...
                      class="title">￥{{product.chongwuNewMoney}}
                </view>
                <view v-else-if="product.chongwuValue"
                      ...
                      class="title">{{product.chongwuValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx;">
                    <text v-if="isAuth('chongwu','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('chongwu','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 寻宠类型 字段 字典表查询方法
            async chongwuTypesSelect(){
                let chongwuTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'chongwu_types',
                });
                chongwuTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.chongwuTypesList = chongwuTypes.data.list
            },
            //当前表的 寻宠类型字段类别搜索
			chongwuTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.chongwuTypes != 0) {
                    params.chongwuTypes = this.chongwuTypes
                }
                let _this = this
                let res = await this.$api.list(`chongwu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chongwu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.chongwuShenheTime=shijian;
                            await _this.$api.update(`chongwu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.chongwuName) {
                    searchForm['chongwuName'] = this.searchForm.chongwuName
                }

                let res = await this.$api.list(`chongwu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.chongwuContent!=null){
                        item.chongwuContent = item.chongwuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
