<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.dongwuLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dongwuLiuyan','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('dongwuLiuyan','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
	...
	<view style="">
    <!--  回复模态框 -->
		<uni-popup ref="replyDeliver" type="center">
            <view
                ...
                <view style="width: 90%;margin: 0 auto;padding-top: 10rpx;">
                    <!-- 回复 -->
                    <text style="display: inline-block;margin-bottom: 10rpx;">回复内容：</text>
                    <textarea v-model='replyData' placeholder="回复内容" style="
                ...
                <view style="display: flex;width: 100%;margin: 20rpx auto;border-top: 1rpx solid #dddddd;">
                    <view style="width: 50%;text-align: center;
							height: 75rpx;line-height: 75rpx;" @tap="closeReply">
                        取消
                    </view>
                    <view @tap="submitReplyData" style="width: 50%;border-left: 1rpx solid #dddddd;text-align: center;height: 75rpx;line-height: 75rpx;color: #0081FF;">
                        确认
                    </view>
                </view>
            </view>
        ...
        <view class="list b-b" v-for="(item, index) in list" :key="index"
            ...
            <view class="wrapper" style="padding: 10rpx 3rpx;margin-left: 25upx;margin-top: 10upx;">
                <view style="font-size: 30rpx;display: flex;justify-content: space-between;">
                    <b>{{item.dongwuName}}</b>
                    <view v-if="item.yonghuId == userId" style="float: right;padding: 10rpx 10rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></view>
                </view>
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.dongwuLiuyanText}}</text>
                        </view>
                ...
                <text style="float: left;padding: 10rpx 10rpx 10rpx;font-size: 25rpx;color:#8799A3;">{{item.insertTime}}</text>
                <uni-icons v-if="isAuth('dongwuLiuyan','回复') && item.dongwuYonghuId == userId" style="float: right;" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
            </view>
        </view>
	</view>
	...
	export default {
		...
		methods: {
			...
			async submitReplyData(){
                if (this.replyData == null || this.replyData == '')
                    return this.$utils.msg('回复不能为空');
                let data = {
                    id: this.replyId ,
                    replyText: this.replyData,
                }
                await this.$api.update(`dongwuLiuyan`, data);
                this.$refs.replyDeliver.close()
                this.mescroll.num = 1
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`dongwuLiuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dongwuLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.dongwuLiuyanShenheTime=shijian;
                            await _this.$api.update(`dongwuLiuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dongwuLiuyanText) {
                    searchForm['dongwuLiuyanText'] = this.searchForm.dongwuLiuyanText
                }

                let res = await this.$api.page(`dongwuLiuyan`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
