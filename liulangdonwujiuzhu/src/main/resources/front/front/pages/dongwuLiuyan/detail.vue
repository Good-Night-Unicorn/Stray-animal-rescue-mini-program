	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dongwuLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dongwuLiuyan', this.dongwuLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dongwuLiuyanPhoto ? this.detail.dongwuLiuyanPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('dongwuLiuyanId', _this.dongwuLiuyanId);
                                // _this.$utils.jump('/pages/dongwuLiuyanYuyue/add-or-update');

								await _this.$api.add('dongwuLiuyanYuyue', {
									yonghuId: _this.user.id,
									dongwuLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/dongwuLiuyanYuyue/list');
							}
						}
					});
				},
