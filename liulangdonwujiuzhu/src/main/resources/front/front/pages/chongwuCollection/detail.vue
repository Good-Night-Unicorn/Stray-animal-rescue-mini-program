	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.chongwuCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('chongwuCollection', this.chongwuCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.chongwuCollectionPhoto ? this.detail.chongwuCollectionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('chongwuCollectionId', _this.chongwuCollectionId);
                                // _this.$utils.jump('/pages/chongwuCollectionYuyue/add-or-update');

								await _this.$api.add('chongwuCollectionYuyue', {
									yonghuId: _this.user.id,
									chongwuCollectionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/chongwuCollectionYuyue/list');
							}
						}
					});
				},
